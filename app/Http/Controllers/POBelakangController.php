...
class POBelakangController extends Controller
{
    ...
    {
        if(Gate::denies('PO Belakang-Update')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        foreach($data['spmb']->spmbdetails as $row) {
            ...
            $detail['spmb_detail_item_price'] = $row->spmb_detail_item_price;
            $detail['spmb_detail_asset_no'] = $row->spmb_detail_asset_no;
            $detail['spmb_detail_note'] = $row->spmb_detail_note;

            $detailvendors = SPMBDetailVendor::with('vendor')->where('spmb_detail_id', $row->spmb_detail_id)->where('spmb_detail_vendor_status', 1)->first();
            $detail['vendor_id'] = $detailvendors->vendor_id;
            $detail['vendor_name'] = $detailvendors->vendor->vendor_name;
        ...
        return view('vendor.material.otherspmb.edit', $data);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($old_detail as $od) {
        	SPMBDetailVendor::where('spmb_detail_id', $od->spmb_detail_id)->delete();
        }
    ...
    public function approveFlowNo2(Request $request, $id)
    {
        ...
        return view('vendor.material.otherspmb.approval_flow_2', $data);
    }
    ...
    public function postApproveFlowNo2(Request $request, $id)
    {
        ...
        $nextFlow = $flow->getNextFlow($this->flow_group_id, $spmb->flow_no, $request->user()->user_id, $request->user()->user_id, $spmb->created_by, $request->user()->user_id);
    ...
    public function approveFlowNo3(Request $request, $id)
    {
        ...
        return view('vendor.material.otherspmb.approval_flow_3', $data);
    }
    ...
    public function approveFlowNo5(Request $request, $id)
    {
        ...
        return view('vendor.material.otherspmb.approval_flow_5', $data);
    }
    ...
    public function postApproveFlowNo5(Request $request, $id)
    {
        ...
        if($request->input('approval')=='1')
        {
            ...
            $flow = new FlowLibrary;
            $nextFlow = $flow->getNextFlow($this->flow_group_id, $spmb->flow_no, $request->user()->user_id, $spmb->created_by, $spmb->created_by, $spmb->created_by);
            ...
            $flow = new FlowLibrary;
            $prevFlow = $flow->getPreviousFlow($this->flow_group_id, $spmb->flow_no, $request->user()->user_id, $spmb->created_by, $spmb->created_by, $spmb->created_by);
