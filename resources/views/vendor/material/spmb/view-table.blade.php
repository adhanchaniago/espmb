	...
	<table id="tabel_detail_spmb" class="table table-bordered table-hover">
		...
		@foreach($spmb->spmbdetails as $detail)
			<tr>
				...
				<td>{{ $detail->unit->unit_code }}</td>
				<td><center>{{ $detail->spmb_detail_qty }}<center></td>
				<td>{{ $detail->spmb_detail_note }}</td>
				<td><center><a title="Lihat Detail" href="javascript:void(0)" class="btn btn-icon command-detail waves-effect waves-circle view-detail-spmb-item" data-detail-id="{{ $detail->spmb_detail_id }}" type="button"><span class="zmdi zmdi-more"></span></a></center></td>
			</tr>
		@endforeach
		</tbody>
	</table>
